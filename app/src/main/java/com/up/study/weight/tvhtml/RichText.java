...
public class RichText extends android.support.v7.widget.AppCompatTextView {
    ...
    public void setRichText(String text) {
        targets.clear();
        if(!text.toString().contains("http")&&(text.toString().contains("doc-dir")||text.toString().contains("user-dir"))){
            String host = TApplication.getInstant().getLoginInfo().getIMGROOT();
            Logger.i(Logger.TAG,"richText添加域名头:"+host);
            text = text.replace("src=\"","src=\""+"http://"+ host+ "/" );
            Logger.i(Logger.TAG,"richText:"+text);
        }
        else {
            Logger.i(Logger.TAG,"richText:全路径");
        }
        ...
        public Drawable getDrawable(String source) {
            ...
            Target target = new Target() {
                ...
                public void onBitmapLoaded(Bitmap bitmap, Picasso.LoadedFrom from) {
                    ...
                    drawable.setBounds(0, 0, bitmap.getWidth()*5/6, bitmap.getHeight()*5/6);
                    urlDrawable.setBounds(0, 0, bitmap.getWidth()*5/6, bitmap.getHeight()*5/6);
            ...
            Picasso.with(getContext())
                    .load(source)
                    .resize(StringUtils.dip2Px(getContext(),250),StringUtils.dip2Px(getContext(),250))
